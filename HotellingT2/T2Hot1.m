...
if n <= p,
   ...
   T2 = nan(r,1);
   if r==1
      S=cov(X(:,1,:));  %Covariance matrix from data matrix X.
      T2=n*(m-mu)*inv(S)*(m-mu)'; %Hotelling's T-Squared statistic.
   elseif r<32
      for i = 1:r
         S = cov(X(:,:,i));
         T2(i) = n*(m(:,:,i)-mu)*inv(S)*(m(:,:,i)-mu)';
      end
   else
      parfor i = 1:r
         T2(i) = n*(m(:,:,i)-mu)*inv(cov(X(:,:,i)))*(m(:,:,i)-mu)';
      end
   end
   
   if n >= 50 %Chi-square approximation.
      method = 'chi2 approximation';
      X2=T2;
